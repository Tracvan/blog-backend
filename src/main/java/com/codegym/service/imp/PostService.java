...
public class PostService implements IPostService {
    ...
    @Override
    public List<PostDTO> findPostByTitle(String input) {
        List<PostDTO> myPostList = postRepository.findByTitle(input);
        for(var i =0; i < myPostList.size(); i++){
            String userDetailAvatar = userService.getUserDetailDTOByUsername(myPostList.get(i).getUsername()).getAvatar();
            myPostList.get(i).setUserAvatar(userDetailAvatar);
            List<CommentDTO> commentDTOList = new ArrayList<>();
    ...
    @Override
    public List<PostDTO> findMyPostByTitle(String title) {
        String username = userService.getCurrentUser().getUsername();
        List<PostDTO> myPostList = postRepository.findMyPostByTitle(title,username);
        for(var i =0; i < myPostList.size(); i++){
            String userDetailAvatar = userService.getUserDetailDTOByUsername(myPostList.get(i).getUsername()).getAvatar();
            myPostList.get(i).setUserAvatar(userDetailAvatar);
            List<CommentDTO> commentDTOList = new ArrayList<>();
